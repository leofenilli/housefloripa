...
final class woo_brands{
	...
	public function eb_add_scripts(){
		...
		wp_register_style('woob-front-end-style', WP_PLUGIN_URL.'/woocommerce-brand/css/front-style.css');
		wp_enqueue_style('woob-front-end-style');
		/* Dropdown css */
		wp_register_style('woob-dropdown-style',  WP_PLUGIN_URL.'/woocommerce-brand/css/msdropdown/dd.css');
		wp_enqueue_style('woob-dropdown-style');
		
		/* Drop Down Js */
		wp_register_script('woob-dropdown-script', WP_PLUGIN_URL.'/woocommerce-brand/js/msdropdown/jquery.dd.js',array( 'jquery' ));
