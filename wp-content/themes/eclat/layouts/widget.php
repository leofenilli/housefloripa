...
else if (in_array($widget->position, array('headerbar', 'toolbar-r' ,'toolbar-l', 'footer', 'offcanvas', 'offcanvas-nav', 'offcanvas-login'))) {
	$panel = 'uk-panel';
}
...
if ($widget->menu) {
    ...
    if((in_array($widget->position, array('toolbar-r', 'toolbar-l', 'offcanvas-nav')) && $widget->showtitle) || in_array($widget->position, array('offcanvas-nav'))){
        $renderer = 'nav';
        $widget->nav_settings["modifier"] = "uk-nav-dropdown";
        $content = '<div data-uk-dropdown><div class="tm-pointer">'.$widget->title.'<span class="tm-icon-arrow-down"></span></div><div class="uk-dropdown'.(in_array($widget->position, array('offcanvas-nav')) ? ' uk-dropdown-flip' : '').'">'.$this['menu']->process($widget, array('pre', 'subnav', $renderer, 'post')).'</div></div>';
    } else {
