...
class MenuWalker extends \Walker_Nav_Menu
{
    ...
    public function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0)
    {
        ...
        // has dop class ?
        /*if ($eclat_class = $warp['config']->get("menus.{$item->ID}.class")) {
            $data['data-menu-class'] = $eclat_class;
        }*/

        // has hide link ?
        if ($hide_link = $warp['config']->get("menus.{$item->ID}.hide_link")) {
            $data['data-menu-hidelink'] = $hide_link;
        }
