...
class WidgetsHelper extends AbstractHelper
{
    ...
    {
        $assignment_styles = $this['config']->get("widgets.{$widget->id}.assignment_styles");

        if (is_array($assignment_styles) && !in_array('*', $assignment_styles) && !in_array($this['config']->get('style'), $assignment_styles)) {
            return false;
        }
