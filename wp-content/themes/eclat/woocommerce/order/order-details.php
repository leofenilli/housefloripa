...
<div class="uk-grid" id="customer_details">
    <div class="uk-width-1-1 uk-width-large-1-3">
        <h2><?php esc_html_e( 'Customer details', 'eclat' ); ?></h2>
        <table class="shop_table customer_details">
            <?php
            if ( $order->billing_email ) {
                echo '<tr><th>' . esc_html__( 'Email:', 'eclat' ) . '</th><td data-title="' . esc_html__( 'Email', 'eclat' ) . '">' . $order->billing_email . '</td></tr>';
            }

            if ( $order->billing_phone ) {
                echo '<tr><th>' . esc_html__( 'Telephone:', 'eclat' ) . '</th><td data-title="' . esc_html__( 'Telephone', 'eclat' ) . '">' . $order->billing_phone . '</td></tr>';
            }
        ...
        <div class="col2-set addresses">
            ...
            <div class="col-1">
                ...
                <header class="title">
                    <h3><?php esc_html_e( 'Billing Address', 'eclat' ); ?></h3>
                </header>
                <address>
                    <?php
                    if ( ! $order->get_formatted_billing_address() ) {
                        esc_html_e( 'N/A', 'eclat' );
                    } else {
            ...
            <div class="col-2">
                ...
                <header class="title">
                    <h3><?php esc_html_e( 'Shipping Address', 'eclat' ); ?></h3>
                </header>
                <address>
                    <?php
    ...
    <div class="uk-width-1-1 uk-width-large-2-3">
        ...
        <h2><?php esc_html_e( 'Order Details', 'eclat' ); ?></h2>
        <table class="shop_table order_details">
            <thead>
                <tr>
                    <th class="product-name"><?php esc_html_e( 'Product', 'eclat' ); ?></th>
                    <th class="product-total"><?php esc_html_e( 'Total', 'eclat' ); ?></th>
                </tr>
            </thead>
            <tbody>
                <?php
                                    ...
                                    <?php
                                        ...
                                        if ( $_product && $_product->exists() && $_product->is_downloadable() && $order->is_download_permitted() ) {
                                            ...
                                            foreach ( $download_files as $download_id => $file ) {
                                                ...
                                                $links[] = '<small><a href="' . esc_url( $file['download_url'] ) . '">' . sprintf( esc_html__( 'Download file%s', 'eclat' ), ( count( $download_files ) > 1 ? ' ' . $i . ': ' : ': ' ) ) . esc_html( $file['name'] ) . '</a></small>';
                                            }
            ...
            <?php
                        ...
                        if ( $has_refund && $key === 'order_total' ) {
                            ...
                            if ( wc_tax_enabled() && 'incl' == $order->tax_display_cart ) {
                                ...
                                if ( ! empty( $tax_del_array ) ) {
                                    $refunded_tax_del .= ' ' . sprintf( esc_html__( '(Includes %s)', 'eclat' ), implode( ', ', $tax_del_array ) );
                                }

                                if ( ! empty( $tax_ins_array ) ) {
                                    $refunded_tax_ins .= ' ' . sprintf( esc_html__( '(Includes %s)', 'eclat' ), implode( ', ', $tax_ins_array ) );
                                }
                            }
                ...
                if ( $has_refund ) { ?>
                    <tr>
                        <th scope="row"><?php esc_html_e( 'Refunded:', 'eclat' ); ?></th>
                ...
                if ( '' != $order->customer_note ) { ?>
                    <tr>
                        <th scope="row"><?php esc_html_e( 'Note:', 'eclat' ); ?></th>
